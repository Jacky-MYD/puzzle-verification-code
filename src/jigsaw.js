...
function createImg (imgSrc, onload) {
  const img = new Image()
  img.crossOrigin = 'Anonymous'
  img.onload = onload
  img.onerror = () => {
   img.setSrc(imgSrc) // 图片加载失败的时候重新加载其他图片
  }
...
class Jigsaw {
  constructor ({ el, width = w, height = h, tranlateX, tranlateY, imgSrc, onSuccess, onFail, onRefresh }) {
    ...
    this.imgSrc = imgSrc || `https://picsum.photos/id/${getRandomNumberByRange(0, 1084)}/${w}/${h}`
    this.el = el
  ...
  draw (img) {
    const { width, height, tranlateX, tranlateY } = this
    // 随机位置创建拼图形状
    this.x = tranlateX != '' ? tranlateX : getRandomNumberByRange(L + 10, width - (L + 10))
    this.y = tranlateY != '' ? tranlateY : getRandomNumberByRange(10 + r * 2, height - (L + 10))
    const randomNuber = Math.floor(Math.random() * 3) + 1
    if (randomNuber == 1) {
  ...
  bindEvents () {
    ...
    const handleDragMove = (e) => {
      ...
      this.blockLeft = (width - 40 - 20) / (width - 40) * moveX
    ...
    const handleDragEnd = (e) => {
      ...
      if (spliced) {
        if (verified) {
          ...
          typeof this.onSuccess === 'function' && this.onSuccess(parseInt(this.blockLeft), this.y)
        } else {
